...
\begin{definition} A category $\mathcal{C}$ consists of:
  \begin{itemize}
  \item A collection of objects $\functionfont{Ob}(\mathcal{C})$
  \item A collection of arrows or morphisims where each arrow $f$ has a domain object $\functionfont{dom}(f)$ and a codomain object $\functionfont{cod}(f)$. These can be defined using $f : A \to B $ to signify $\functionfont{dom}(f) = A$ and $\functionfont{cod}(f) = B$
  \item A composition operator assigning to each pair of arrows $f$ and $g$, with $cod(f) = dom(g)$, a composite arrow $g \circ f : \functionfont{dom}(f) \to \functionfont{cod} (g)$ satisfying the following associative law:
    \[
...
A functor can be viewed as a structured “translation” between categories. 
It assigns to each object in a category $\mathcal{C}$ an object in a category $\mathcal{D}$, 
and to each morphism in $\mathcal{C}$ a morphism in $\mathcal{D}$, 
in such a way that identities and composition are preserved.
...
to relate their outputs while respecting the internal structure of $\mathcal{C}$. Formally:





\subsection{\centering Universal Constructions}
\begin{definition}
