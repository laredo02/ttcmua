...
consists of
\begin{enumerate}
    \item an assignment of an object $F(X) \in \mathrm{Ob}(\mathcal{D})$ to every object $X \in \mathrm{Ob}(\mathcal{C})$,
    \item an assignment of a morphism $F(f) : F(X) \to F(Y)$ in $\mathcal{D}$ to every morphism $f : X \to Y$ in $\mathcal{C}$,
\end{enumerate}
such that the following hold:
\begin{align*}
    F(\mathrm{id}_X) &= \mathrm{id}_{F(X)} 
    && \text{for all } X \in \mathrm{Ob}(\mathcal{C}), \\
    F(g \circ f) &= F(g) \circ F(f) 
    && \text{for all } f : X \to Y, \; g : Y \to Z \text{ in } \mathcal{C}.
\end{align*}
\end{definition}
