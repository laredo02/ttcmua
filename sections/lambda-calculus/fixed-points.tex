...
\begin{example}
  ...
  \]
\end{example}
\begin{definition} Turing combinator
\[
  \Theta \triangleq 
...
\begin{example} Implementing factorial using the acquired toolkit:
  \[
    \underline{\texttt{fact}}\; n
    = \texttt{if\ then\ else}\;(\texttt{iszero}\; n)\;(1)\;(\texttt{mult}\; n\;(\underline{\texttt{fact}}\;(\texttt{pred}\; n))).
  \]
\end{example}
The evident problem with a recursive definition such as this one is that the term to be defined, $\texttt{fact}$, appears both on the left- and the right-hand side. This means finding $\texttt{fact}$ requires solving an equation!

We now apply our newfound knowledge of how to solve fixed point equations in the lambda calculus. We start by rewriting the problem slightly:
\[
  \texttt{fact}
  = \lambda n.\; \texttt{if\ then\ else}\;(\texttt{iszero}\; n)\;(1)\;(\texttt{mult}\; n\;(\texttt{fact}\;(\texttt{pred}\; n))),
\]
\[
  \texttt{fact}
  = \bigl(\lambda f.\,\lambda n.\; \texttt{if\ then\ else}\;(\texttt{iszero}\; n)\;(1)\;(\texttt{mult}\; n\;(f\;(\texttt{pred}\; n)))\bigr)\;\texttt{fact}.
\]

Let us temporarily write $F$ for the term
\[
  F = \lambda f.\,\lambda n.\; \texttt{if\ then\ else}\;(\texttt{iszero}\; n)\;(1)\;(\texttt{mult}\; n\;(f\;(\texttt{pred}\; n))).
\]

Then the last equation becomes $\texttt{fact} = F\,\texttt{fact}$, which is a fixed point equation.
We can solve it up to $\beta$-equivalence by letting
\[
  \texttt{fact} = \Theta F
  = \Theta\bigl(\lambda f.\,\lambda n.\; \texttt{if\ then\ else}\;(\texttt{iszero}\; n)\;(1)\;(\texttt{mult}\; n\;(f\;(\texttt{pred}\; n)))\bigr).
\]
...
\subsection{\centering Conclusions}
We have covered some of the most important notions that surround the untyped \lcalc. It has been shown to satisfy the Church-Rosser property of confluence, it has also been shown that this property is not the strongest of its kind. It could be argued it is in fact the weakest. For this reason, it is in our best interest to add some spice to the \lcalc \ by means typing. This is to try and make it comply with some stronger requirements than the Church-Rosser property. There exist several typing flavors, the list shows the three basic categories:
\begin{itemize}
...
\item \textbf{Simply Typed}: Every expression is annotated with a type. This would be equivalent to the usual set theoretic approach to computation where functions have well defined domain and codomain.
\item \textbf{Polymorphically Typed}: This is an intermediate situation, where we may specify the form of the type $X \to  X$, without actually specifying $X$.
\end{itemize}
