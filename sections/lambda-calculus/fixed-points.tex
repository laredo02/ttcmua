...
We have seen that not all terms are normalizing. The most straight forward way to test this, is to check whether there exists some term that reduces to itself, effectively creating a loop in the $\beta$-reduction chain so to speak. In more formal terms this means there exists some $M$ such that \( M \curly^n M \) with $ n > 0 $. As already seen, \( \Omega \equiv (\la x . \, x \, x) (\la x . \, x \, x) \) fullfills this requirement. Let us recall what goes on while $\beta$-reducing this term:
\[
...
\end{theorem}
\begin{proof} Given $L \in \La$, define
  \(
    M := (\la x . \, L \, (x \, x))(\la x . \, L \, (x \, x))
  \),
  since $M$ is reducible:
  \begin{align*}
    M 
...
Now that nameless self-reference has been proven to exist within the \lcalc, we take a step forward and introduce a few of the most important fixed point combinators and explore their different natures.
...
\end{example}
\begin{remark}
  The extra $\lambda v$ in the final form of $Z\,F$ is essential to delay evaluation in a strict environment. Without it, the recursive call would be applied immediately, and cause non-termination in a call-by-value setting.
\end{remark}
...
\begin{example} Implementing factorial using the acquired toolkit:
  \[
    ...
    = \lambda n.\; \texttt{if}\;(\texttt{iszero}\; n)\;(\overline{\texttt{1}})\;(\texttt{mult}\; n\;(\underline{\texttt{fact}}\;(\texttt{pred}\; n))),
  \]
\end{example}
...
\[
  \texttt{fact}
  = \overbrace{\bigl(\lambda f.\,\lambda n.\; \texttt{if}\;(\texttt{iszero}\; n)\;(1)\;(\texttt{mult}\; n\;(f\;(\texttt{pred}\; n)))\bigr)}^{F}\;\texttt{fact}.
\]
...
\begin{note}
  The latter implementation exhibits a striking syntactic resemblance to that of the Lisp family of languages.
\end{note}
