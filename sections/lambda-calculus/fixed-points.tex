...
\begin{example} Implementing factorial using the acquired toolkit:
  \[
    \texttt{fact}\; n
    = \texttt{if\ then\ else}\;(\texttt{iszero}\; n)\;(1)\;(\texttt{mult}\; n\;(\texttt{fact}\;(\texttt{pred}\; n))).
  \]
\end{example}
The evident problem with a recursive definition such as this one is that the term to be defined, $\texttt{fact}$, appears both on the left- and the right-hand side. In other words, to find $\texttt{fact}$ requires solving an equation!
