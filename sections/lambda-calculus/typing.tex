...
In the untyped \(\lambda\)-calculus, not all terms reach a normal form, as seen in \ref{sec:normalization}; some computations can run forever. This makes reasoning about programs more difficult and motivates the study of typed variants. In the following, we provide a few of the most relevant properties of this minimally typed variant, their proofs are given in \cite{nederpelt2014:type-theory} and also \cite{Barendregt1992}.
...
The Church–Rosser property holds for $\lambda^\to$.
\end{theorem}

\begin{corollary}
\label{cor:church-rosser}
Suppose \( M =_\beta N \). Then there exists \( L \) such that
\[
...
\begin{theorem}[Strong Normalisation Theorem (Termination)]
\label{thm:strong-normalisation}
Every legal term \( M \) in $\lambda^\to$ is strongly normalising.
\end{theorem}
