...
\subsubsection{\centering The Language of \lCalc}
...
Since we are dealing with a formal language, it is in our benefit to introduce a few other objects with the aim of defining a grammar to generate it:
\begin{itemize}
...
\end{itemize}
Our aim now, to generate the set $\La$,  to do this, we will make use of a grammar. When dealing with languages, notation and similar concepts, Backus-Naur Form, often shortened to BNF, is one of the more complete alternatives:
\begin{itemize}
...
\end{align*}
\begin{notation}
  Throughout the document BNF will be used to define language specifics. In other words, it will be used when details matter. The times when notational rigor is beyond the point, abstract syntax will be used to convey more general concepts that do not require a standard notation.
\end{notation}
\begin{definition} Taking advantage of BNF notation, an alternative definition for $\La$ would be:
  ...
  Through use of inductive types types\footnote{Inductive Data Types are built from sums (either this or that) of products (this and that). The main point is that they allow self-reference and support pattern matching.}, this grammar can be implemented in Lean4:
\end{definition}
\begin{remark}
...
\end{itemize}
All these are just to keep notation straight-forward, the formal definition does not leave precedence under-specified thanks to parenthesis. Those interested in the implementation need not worry, as Lean4 enforces an explicit order on constructors.
...
From the ground up, we begin define the concept of subterm, since the definition of the language is self-referential and terms might contain other terms.
\begin{definition} Given $T \in \La$, $\functionfont{Sub} : \La \to \mathcal{P}(\Vset)$ maps a term to the set of it's subterms.
  ...
  \end{align*}
  Where $x \in \Vset$ and $M, N \in \La $. In Lean4:
\end{definition}
\begin{remark}
...
\begin{example} If we mistakenly substitute $x$ by \( (\la w . \, w \, y) \) in \( (\la y . \, y \, x ) \) using rule five, we get:
  ...
  \]
  This is problematic since the free variable $y$ became bound after the rename. Free and bound variables should mantain their freedom or their boundedness after the rename.
\begin{note}
  This is essentially why it is called Capture Avoiding Substitution and not simply Substitution.
\end{note}
After introducing rule six, we rename the binding variable to avoid the name clash:
  \[
...
\begin{proof}
\orange{structural induction on $M$.}
\end{proof}
...
\end{example}
\begin{remark}
  \aequiv \ comes naturally once substitution is defined, since the names of bound variables are just placeholders that will eventually hold an argument, thus getting replaced.
\end{remark}
...
\begin{definition} Multi-step \bred. We write $M \step^* N$ if and only if there exists a sequence of one-step $\beta$-reductions starting from $M$ and ending at $N$.\footnote{The convention in the literature is to use $\rightarrow_\beta$ for \bred s $\twoheadrightarrow_\beta$ for multi-step \bred s. I prefer to use $\curly$ and $\curly^n$ since this allows me to use the superscript to note the number of steps in the process \bred \ e.g. $ (\la f. \la x . \, x) \, M \, N \curly^2 $ N. Instead of $\curly^n$ I use $\curly^*$ whenever the number of steps is undefined.}
...
\end{definition}
\begin{note}
  Multi-step $\curly^*$ is the reflexive transitive closure of $\curly$.
\end{note}
The process of succesively $\beta$-reducing a term leads raises the question of whether all terms can be reduced indefinitly. We know the answer to this, since for instance, variables in $\Vset$ cannot be reduced. The subsequent question is then, Do all all terms have some kind of normal form that cannot be reduced further? This is answered in Section \ref{sec:recursion-fixed-points}. To this end we introduce a formal definition of normal form that will allow us to gain insight into the normalization properties of the \lcalc.
...
The last and indeed the least important notion of equivalence. It refers to a program being equivalent to itself wrapped with an identity function:
...
\end{definition}
\begin{note}
  Two terms are observationally equivalent if they cannot
  be distinguished by any context that can be papplied to them. $\eta$-equivalence can be understood as a form of weak extensionality, since it does not fully encode observational equivalence.
\end{note}

\orange{
  Add something about normalization properties and how they dont ensure termination. this is important for programming blah blah blah...
  This is why we introduce Types and Type Systems... This notion is similar to that of restricting the domain and codomain of a function.
}
