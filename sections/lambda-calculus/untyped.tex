...
The Lambda Calculus together with Type Theory lay the theoretical groundwork that shaped the design of languages like ML or the more recent Haskell. By itself, the untyped \lcalc \ modeled the first versions of Lisp. More advanced type systems than those present in general purpose programming languages enable computers to verify mathematical statements and produce rigorous proofs.



\section{\centering The Language}
...
\[ \la x . \, x \iff ( \la x . \, x ) \qquad \la x. M \, N \iff \la x. (M \, N) \qquad  M \, N \, P \iff (M \, N) \, P \qquad \la x. \la y. f(x,y) \iff \la xy. f(x,y) \]
...
\begin{example} A simple equivalence class and two samples of not $\alpha$-equivalent \ pairs.
  ...
  \]
  Remember that terms are $\alpha$-equivalent \ only when bounded names are properly substituted.
\end{example}
\begin{remark}
...
\begin{note}
  Variable naming is a nightmare. De Bruijn indexing is a way of methodically assigning numbers to variable names, indicating how many binders away a variables binder is. This system eliminates the need for $\alpha$ equivalence to exist. See \cite{deBruijn1972}.
  \[
