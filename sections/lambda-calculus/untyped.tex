...
\subsection{\centering The Language of \lCalc}
...
\begin{example} \label{ex:lambda-terms} Some examples of valid $\la$ terms generated using the grammar in \ref{def:lambda-terms-1}:
  \( y \),
  \( (\la x. (x x)) \),
  \( (\la x. (\la y. x)) \),
  \( (((\la x. (x y)) (\la y. y)) z) \).
...
\begin{definition} Given $T \in \La $, $\functionfont{FV} : \La \to \mathcal{P}(\Vset) $ outputs the set of free variables for $T$:
  ...
  \end{align*}
  Where $ x \in \Vset $ and $ M, N \in \La $, and $\mathcal{P}$ denotes the power set. In Haskell:
  \begin{minted}[fontsize=\small]{haskell}
...
capAvoidSubs same@(Var y) x n
  | y == x = n
  | otherwise = same
capAvoidSubs (App t1 t2) s t3 =
  App (capAvoidSubs t1 s t3) (capAvoidSubs t2 s t3)
capAvoidSubs (Abs y m) x n
  | y == x = (Abs y m)
  | otherwise =
      if Set.member y (freeVars n)
      then
  ...
  where
    ...
    freshVar s terms =
      ...
      \begin{note}
        \texttt{freshVar} generates a new name for binding variables such that \( z \notin \FV { M } \cup \FV { N } \). In this instance we have chosen to add single quotes as long as the variable name is contained in \( \FV { M } \cup \FV { N } \).
      \end{note}
  \end{definition}
